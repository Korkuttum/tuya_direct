"""DataUpdateCoordinator for Tuya Direct."""
...
from .generic_model import (
    build_mapping_from_tuya_model,
    create_empty_mapping,
    extend_mapping_from_data,
)
...
class TuyaScaleDataUpdateCoordinator(DataUpdateCoordinator):
    """Class to manage fetching Tuya device data with instant updates."""
    ...
    async def get_device_info(self) -> dict:
        ...
        if self.connection_type == "cloud":
            ...
            self.device_name = f"Tuya Direct Local {self.device_id[-6:]}"
            self.device_info = DeviceInfo(
                identifiers={(DOMAIN, self.device_id)},
                name=self.device_name,
                manufacturer=DEFAULT_MANUFACTURER,
    ...
    async def get_device_model(self) -> dict:
        ...
        if False and cached_model_id and cached_for_device == self.device_id:
            _LOGGER.info(
                "✅ model_id config_entry cache'den alındı: %s → buluta bağlanılmıyor",
                cached_model_id
            )
            self.model_id = cached_model_id
            self.model_mapping = create_empty_mapping(self.model_id)
        ...
        try:
            ...
            if result.get('success', False):
                ...
                self.model_mapping = build_mapping_from_tuya_model(model_info)
                self._build_dp_mapping()
        ...
        # Default fallback
        self.model_mapping = create_empty_mapping("generic")
        self._build_dp_mapping()
    ...
    def extend_model_mapping_from_data(self) -> None:
        """Add missing live DPs and raw field sensors to the dynamic model."""
