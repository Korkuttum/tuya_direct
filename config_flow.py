"""Config flow for Tuya Direct integration."""
...
async def validate_input(hass: HomeAssistant, data: dict, connection_type: str) -> dict:
    ...
    if connection_type == "cloud":
        ...
        return {"title": f"Tuya Direct ({data[CONF_DEVICE_ID]})"}
        ...
        return {"title": f"Tuya Direct Local ({data[CONF_DEVICE_ID]})"}
...
class TuyaHeatpumpConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Tuya Direct."""
